...
from .layers import DenseBlock, Net, ResidualBlock, TFSamepaddingLayer, UpSample2x
from .utils import crop_op


def load_hovernet(weights: str) -> Net:
    # TODO: Add more pretrained weights
    urls = {
        "pannuke": "https://nextcloud.centralesupelec.fr/s/RziGGRSLzNKGN2a/download/hovernet_fast_pannuke_type_tf2pytorch.tar",
    }
    type_dicts = {
        "pannuke": {
            1: "Neoplastic",
    ...
    state_dict = torch.hub.load_state_dict_from_url(urls[weights])
    hovernet = HoVerNet(mode="fast", nr_types=6)
    hovernet.load_state_dict(state_dict["desc"])
    return hovernet, type_dicts[weights]
